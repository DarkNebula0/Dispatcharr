...
def refresh_vod_content(account_id):
    ...
    try:
        ...
        # Automatically generate STRM files for all movies in this account
        logger.info(f"Starting automatic STRM generation for all movies in account {account.name}")
        try:
            # Get all movie IDs that have relations for this account
            movie_relations = M3UMovieRelation.objects.filter(
                m3u_account=account
            ).values_list('movie_id', flat=True)
            
            if movie_relations:
                movie_ids = list(movie_relations)
                logger.info(f"Generating STRM files for {len(movie_ids)} movies")
                # Call batch_generate_movie_strms asynchronously
                batch_generate_movie_strms.delay(account_id, movie_ids=movie_ids)
                logger.info(f"Queued STRM generation for {len(movie_ids)} movies")
            else:
                logger.info("No movies found for STRM generation")
        except Exception as e:
            logger.error(f"Error queuing movie STRM generation: {str(e)}", exc_info=True)
            # Don't fail the entire VOD refresh if STRM generation fails
        ...
        # Automatically refresh episodes only for series that have never been fetched (initial sync)
        logger.info(f"Starting automatic episode refresh for new series in account {account.name}")
        try:
            # Get only series that have never had episodes fetched (last_episode_refresh is NULL)
            series_relations = M3USeriesRelation.objects.filter(
                m3u_account=account,
                last_episode_refresh__isnull=True
            ).values_list('series_id', flat=True)
            
            if series_relations:
                series_ids = list(series_relations)
                logger.info(f"Refreshing episodes for {len(series_ids)} series (initial fetch)")
                # Call batch_refresh_series_episodes asynchronously
                batch_refresh_series_episodes.delay(account_id, series_ids=series_ids)
                logger.info(f"Queued episode refresh for {len(series_ids)} series")
            else:
                logger.info("No new series found for episode refresh (all series already have episodes)")
        except Exception as e:
            logger.error(f"Error queuing episode refresh: {str(e)}", exc_info=True)
            # Don't fail the entire VOD refresh if episode refresh fails
...
def process_movie_batch(account, batch, categories, relations, scan_start_time=None):
    ...
    # Disable signals during batch operations to prevent duplicate STRM generation
    # STRM files will be generated by batch_generate_movie_strms task after refresh completes
    from django.db.models.signals import post_save
    from apps.vod.signals import movie_post_save
    
    # Disconnect signal temporarily
    post_save.disconnect(movie_post_save, sender=Movie)
    ...
    try:
        with transaction.atomic():
            ...
            # Note: STRM file generation is handled by batch_generate_movie_strms task
            # which is called after refresh_vod_content completes. This avoids duplicate
            # generation and improves performance by batching all STRM operations.
...
def refresh_series_episodes(account, series, external_series_id, episodes_data=None):
    ...
    try:
        if not episodes_data:
            # Check if episodes already exist in DB for this account
            existing_episodes = Episode.objects.filter(
                series=series,
                m3u_relations__m3u_account=account
            ).exists()
            
            # Get the series relation to check last_episode_refresh
            series_relation = M3USeriesRelation.objects.filter(
                series=series,
                m3u_account=account
            ).first()
            
            # Check if we should skip fetching (episodes exist)
            if existing_episodes and series_relation and series_relation.last_episode_refresh:
                # Episodes exist and were fetched before - no need to refresh
                # Episodes are static once released, so no time-based check needed
                logger.info(f"Skipping episode fetch for series {series.name} - episodes already exist (last refresh: {series_relation.last_episode_refresh})")
                # Episodes are cached, but we still need to update the series relation metadata
                # to mark that we checked (even though we didn't fetch)
                if series_relation:
                    ...
                    # Don't update last_episode_refresh since we didn't actually refresh
                    series_relation.save()
                return
            
            # Episodes are missing or never fetched, fetch from provider
            if existing_episodes:
                logger.info(f"Fetching episodes for series {series.name} from provider (episodes exist but never marked as fetched)")
            else:
                logger.info(f"Fetching episodes for series {series.name} from provider (episodes missing)")
            with XtreamCodesClient(
                account.server_url,
                account.username,
                account.password,
...
def batch_process_episodes(account, series, episodes_data, scan_start_time=None):
    ...
    """
    # Disable signals during batch operations to prevent duplicate STRM generation
    # STRM files will be generated by signals (for individual saves) or batch operations
    from django.db.models.signals import post_save
    from apps.vod.signals import episode_post_save
    
    # Disconnect signal temporarily
    post_save.disconnect(episode_post_save, sender=Episode)
    
    if not episodes_data:
        # Reconnect signal before returning
        post_save.connect(episode_post_save, sender=Episode)
        return
    ...
    try:
        ...
        with transaction.atomic():
            ...
            # Note: STRM file generation for episodes is handled by signals (for individual saves)
            # or by batch_refresh_series_episodes which processes episodes in batch.
            # This avoids duplicate generation and improves performance.
        ...
        # Always reconnect signal after batch processing, even if errors occur
        post_save.connect(episode_post_save, sender=Episode)
...
def batch_refresh_series_episodes(account_id, series_ids=None):
    ...
    If series_ids is None, refresh only series that have never been fetched (last_episode_refresh is NULL).
    If series_ids is provided, refresh those specific series (manual refresh).
    Episodes are only fetched once on initial sync - no automatic time-based refreshes.
    """
    try:
        account = M3UAccount.objects.get(id=account_id, is_active=True)
        ...
        if series_ids:
            # Manual refresh - refresh specified series
            series_relations = M3USeriesRelation.objects.filter(
                m3u_account=account,
                series__id__in=series_ids
            ).select_related('series')
        else:
            # Auto-refresh only for series that have never been fetched (initial sync)
            series_relations = M3USeriesRelation.objects.filter(
                m3u_account=account,
                last_episode_refresh__isnull=True
            ).select_related('series')
        ...
        with XtreamCodesClient(
            ...
            for relation in series_relations:
                try:
                    ...
                    # Check if refresh is needed (only if episodes are missing or never fetched)
                    needs_refresh = (
                        not existing_episodes or
                        not relation.last_episode_refresh  # Never fetched before
                    )
                    # NO time-based check - episodes are static once released
                    
                    if needs_refresh:
                        logger.info(f"Refreshing episodes for series {relation.series.name} (episodes missing or never fetched)")
                        ...
                        logger.info(f"Skipping episode refresh for series {relation.series.name} - episodes already exist (last refresh: {relation.last_episode_refresh})")
                        skipped_count += 1
                except Exception as e:
                    logger.error(f"Error refreshing episodes for series {relation.series.name}: {str(e)}")

        logger.info(f"Batch episode refresh completed: {refreshed_count} refreshed, {skipped_count} skipped (cached)")
        
        # Automatically generate STRM files for all episodes in this account
        logger.info(f"Starting automatic STRM generation for all episodes in account {account.name}")
        try:
            # Get all episode IDs that have relations for this account
            episode_relations = M3UEpisodeRelation.objects.filter(
                m3u_account=account
            ).values_list('episode_id', flat=True)
            
            if episode_relations:
                episode_ids = list(episode_relations)
                logger.info(f"Generating STRM files for {len(episode_ids)} episodes")
                # Call batch_generate_episode_strms asynchronously
                batch_generate_episode_strms.delay(account_id, episode_ids=episode_ids)
                logger.info(f"Queued STRM generation for {len(episode_ids)} episodes")
            else:
                logger.info("No episodes found for STRM generation")
        except Exception as e:
            logger.error(f"Error queuing episode STRM generation: {str(e)}", exc_info=True)
            # Don't fail the entire episode refresh if STRM generation fails
        
        return f"Batch episode refresh completed: {refreshed_count} refreshed, {skipped_count} skipped (cached)"
...
@shared_task
def batch_generate_movie_strms(account_id, movie_ids=None):
    """
    Batch generate STRM files for multiple movies.
    If movie_ids is None, generate STRM files for all movies with M3U relations for this account.
    """
    try:
        account = M3UAccount.objects.get(id=account_id, is_active=True)

        if account.account_type != M3UAccount.Types.XC:
            logger.warning(f"Movie STRM generation called for non-XC account {account_id}")
            return "Movie STRM generation only available for XtreamCodes accounts"
        ...
        if movie_ids:
            ...
            # Generate STRM files for all movies with relations for this account
            movie_relations = M3UMovieRelation.objects.filter(
                m3u_account=account
            ).select_related('movie')

        logger.info(f"Batch generating STRM files for {movie_relations.count()} movies")

        from .strm_generator import generate_movie_strm

        generated_count = 0
        skipped_count = 0
        error_count = 0

        # Process in chunks to avoid very long transactions
        chunk_size = 100
        relations_list = list(movie_relations)
        
        for i in range(0, len(relations_list), chunk_size):
            chunk = relations_list[i:i + chunk_size]
            with transaction.atomic():
                for relation in chunk:
                    try:
                        # relation.movie is guaranteed to have m3u_relations since we filtered by M3UMovieRelation
                        generate_movie_strm(relation.movie)
                        generated_count += 1
                    except Exception as e:
                        error_count += 1
                        logger.error(f"Error generating STRM for movie {relation.movie.name}: {str(e)}")
                        # Continue processing other movies in the chunk even if one fails

        logger.info(f"Batch STRM generation completed: {generated_count} generated, {skipped_count} skipped, {error_count} errors")
        return f"Batch STRM generation completed: {generated_count} generated, {skipped_count} skipped, {error_count} errors"

    except Exception as e:
        logger.error(f"Error in batch movie STRM generation for account {account_id}: {str(e)}")
        return f"Batch STRM generation failed: {str(e)}"


@shared_task
def batch_generate_episode_strms(account_id, episode_ids=None):
    """
    Batch generate STRM files for multiple episodes.
    If episode_ids is None, generate STRM files for all episodes with M3U relations for this account.
    """
    try:
        account = M3UAccount.objects.get(id=account_id, is_active=True)

        if account.account_type != M3UAccount.Types.XC:
            logger.warning(f"Episode STRM generation called for non-XC account {account_id}")
            return "Episode STRM generation only available for XtreamCodes accounts"
        ...
        if episode_ids:
            ...
            # Generate STRM files for all episodes with relations for this account
            episode_relations = M3UEpisodeRelation.objects.filter(
                m3u_account=account
            ).select_related('episode', 'episode__series')

        logger.info(f"Batch generating STRM files for {episode_relations.count()} episodes")

        from .strm_generator import generate_episode_strm
        ...
        # Process in chunks to avoid very long transactions
        chunk_size = 100
        relations_list = list(episode_relations)
        ...
        for i in range(0, len(relations_list), chunk_size):
            ...
            with transaction.atomic():
                for relation in chunk:
                    try:
                        # relation.episode is guaranteed to have m3u_relations since we filtered by M3UEpisodeRelation
                        generate_episode_strm(relation.episode)
                        generated_count += 1
                    except Exception as e:
                        error_count += 1
                        logger.error(f"Error generating STRM for episode {relation.episode}: {str(e)}")
                        # Continue processing other episodes in the chunk even if one fails

        logger.info(f"Batch episode STRM generation completed: {generated_count} generated, {skipped_count} skipped, {error_count} errors")
        return f"Batch episode STRM generation completed: {generated_count} generated, {skipped_count} skipped, {error_count} errors"

    except Exception as e:
        logger.error(f"Error in batch episode STRM generation for account {account_id}: {str(e)}")
        return f"Batch episode STRM generation failed: {str(e)}"
...
def cleanup_orphaned_vod_content(stale_days=0, scan_start_time=None, account_id=None):
    ...
    # Clean up orphaned STRM files
    strm_cleanup_result = cleanup_orphaned_strm_files(account_id=account_id)
    ...
    result = (f"Cleaned up {stale_movie_count} stale movie relations, "
              ...
              f"{orphaned_movie_count} orphaned movies, "
              f"{orphaned_series_count} orphaned series. {strm_cleanup_result}")
...
@shared_task
def cleanup_orphaned_strm_files(account_id=None):
    """
    Clean up orphaned STRM files:
    - Remove STRM files for movies/episodes that no longer have M3U relations
    - Remove STRM file records where the file is missing from disk
    - Remove actual files from disk when they're no longer needed
    """
    import os
    from django.contrib.contenttypes.models import ContentType
    from .models import STRMFile, Movie, Episode
    
    logger.info("Starting cleanup of orphaned STRM files")
    ...
    missing_file_records_count = 0
    
    # Use iterator to process in chunks and avoid loading all records into memory
    strm_files = STRMFile.objects.all().iterator(chunk_size=1000)
    
    if account_id:
        # Filter to only movies/episodes for this account
        movie_ids = set(M3UMovieRelation.objects.filter(m3u_account_id=account_id).values_list('movie_id', flat=True))
        episode_ids = set(M3UEpisodeRelation.objects.filter(m3u_account_id=account_id).values_list('episode_id', flat=True))
    else:
        # Get all movies/episodes with relations
        movie_ids = set(Movie.objects.filter(m3u_relations__isnull=False).values_list('id', flat=True))
        episode_ids = set(Episode.objects.filter(m3u_relations__isnull=False).values_list('id', flat=True))
    
    for strm_file in strm_files:
        try:
            content_id = strm_file.content_id
            should_delete = False
            file_missing = False
            
            # Check if content object still exists (might have been deleted)
            try:
                content_obj = strm_file.content_object
                if content_obj is None:
                    # Content object was deleted, mark for cleanup
                    should_delete = True
                    logger.debug(f"Content object for STRM file {strm_file.id} no longer exists, marking for deletion")
            except Exception:
                # Content object doesn't exist, mark for cleanup
                should_delete = True
                logger.debug(f"Content object for STRM file {strm_file.id} (content_id={content_id}) no longer exists, marking for deletion")
            
            # Check if content still has relations (only if object exists)
            if not should_delete:
                if strm_file.content_type == movie_content_type:
                    if content_id not in movie_ids:
                        should_delete = True
                        logger.debug(f"Movie {content_id} no longer has M3U relations, marking STRM file for deletion")
                elif strm_file.content_type == episode_content_type:
                    if content_id not in episode_ids:
                        should_delete = True
                        logger.debug(f"Episode {content_id} no longer has M3U relations, marking STRM file for deletion")
            
            # Check if file exists on disk
            if strm_file.strm_path and not os.path.exists(strm_file.strm_path):
                file_missing = True
                should_delete = True
                logger.debug(f"STRM file missing from disk: {strm_file.strm_path}")
            
            if should_delete:
                # Delete files from disk if they exist
                if strm_file.strm_path and os.path.exists(strm_file.strm_path):
                    try:
                        os.remove(strm_file.strm_path)
                        deleted_files_count += 1
                        logger.debug(f"Deleted STRM file: {strm_file.strm_path}")
                    except Exception as e:
                        logger.warning(f"Failed to delete STRM file {strm_file.strm_path}: {e}")
                
                if strm_file.nfo_path and os.path.exists(strm_file.nfo_path):
                    try:
                        os.remove(strm_file.nfo_path)
                        logger.debug(f"Deleted NFO file: {strm_file.nfo_path}")
                    except Exception as e:
                        logger.warning(f"Failed to delete NFO file {strm_file.nfo_path}: {e}")
                ...
                if file_missing:
                    missing_file_records_count += 1
                    
        except Exception as e:
            logger.error(f"Error cleaning up STRM file {strm_file.id}: {e}", exc_info=True)
    
    result = f"STRM cleanup: {deleted_files_count} files deleted, {deleted_records_count} records deleted ({missing_file_records_count} missing files)"
    logger.info(result)
    return result
